...
function filterByValue(arr, key) {
    let newArr = arr.filter(
        (val) => {
            return val[key] === true || val['first'] === key || val['last'] === key;
        }
    )
    return newArr;
}

console.log(filterByValue([{first: 'Elie', last:"Schoppik"}, {first: 'Tim', last:"Garcia", isCatOwner: true}, {first: 'Matt', last:"Lane"}, {first: 'Colt', last:"Steele", isCatOwner: true}], 'Elie'));
...
function removeVowels(str) {
    ...
    let newArr = arr.filter(
        (char) => {
            return char !== 'e' && char !== 'i' && char !== 'a' && char !== 'u' && char !== 'o'; 
        }
    )
